...
class FirebaseEmergencyContactService {
  ...
  Future<List<EmergencyContactModel>> getAllEmergencyContacts(String userId) async {
    try {
      // Get built-in contacts
      final List<EmergencyContactModel> builtInContacts = EmergencyContactModel.getBuiltInContacts(userId);
      
      // Get user's personal contacts
      final QuerySnapshot userContactsSnapshot = await _firestore
      ...
      final List<EmergencyContactModel> userContacts = userContactsSnapshot.docs
          .map((doc) => EmergencyContactModel.fromJson(doc.data() as Map<String, dynamic>))
          .toList();
  ...
  Stream<List<EmergencyContactModel>> getEmergencyContactsStream(String userId) {
    try {
      // Get built-in contacts
      final List<EmergencyContactModel> builtInContacts = EmergencyContactModel.getBuiltInContacts(userId);
      ...
      return _firestore
          ...
          .snapshots()
          .map((snapshot) {
            final List<EmergencyContactModel> userContacts = snapshot.docs
                .map((doc) => EmergencyContactModel.fromJson(doc.data() as Map<String, dynamic>))
                .toList();
  ...
  Future<bool> addEmergencyContact(EmergencyContactModel contact) async {
    try {
      String contactId = contact.id.isEmpty ? _firestore.collection(_collectionPath).doc().id : contact.id;
